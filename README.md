...
1. 개요
2. 구조
   1. 구현에 참고한 설계 방법론 및 기술
   2. 컴포넌트별 상세 설명
   3. CQRS 상세
   4. 메시지 처리 상세
   5. 운영 인프라 상세
3. 개발 중 발생한 특별한 이슈
   1. 로그인 처리시 컴포넌트간 동기화 문제
   2. 거의 동시에 들어오는 요청에 대한 transaction 처리
   3. Pod 단위 로그 수집
4. 사용 기술
...
클로즈메모는 CQRS 기반이기 때문에 갱신과 조회가 DB 단위에서 분리된 구조입니다. 그리고 비동기적으로 일기 전용 DB를 갱신하기 때문에 때문에 DB간 동기화에 시간이 걸립니다. 그래서 짧은 기간 내에 동기화가 필요한 경우 문제가 발생할 수 있습니다.

로그인 처리가 그러한 경우입니다. 특히 "네이버 아이디로 로그인" 같이 외부 로그인 API 를 사용하는 경우 응답시간이 일정하지 않아서 동기화 되기까지 단순히 대기 시간을 늘리는 방식은 적절하지 않습니다. (불필요한 대기시간 증가)

아래는 현재 클로즈메모의 로그인 구조에 대한 설명입니다.

| ![login](./img/login.png) |
|:---:|
| 클로즈메모의 로그인 구조 (6 의 경우 비동기) |

위에서 볼 수 있듯이 DB 두 개가 분리되어 있기 때문에 토큰 정보 DB 양쪽 모두 저장되기 까지 시간이 걸립니다.

이러한 토큰 기반 로그인 처리에서 `command` 에서 발급한 토큰이 `query` 에 의해 읽기 전용 DB에 반영되기 전에 브라우저의 조회요청(위에서 10번 요청)이 들어온다면 요청에 대한 적절한 권한을 얻을 수 없습니다.

#### 해결방법

읽기 전용 DB 까지 저장이 될 때까지 응답하지 않고 기다리는 것이 필요한 경우, `command` 에서 동기화 완료 메시지를 받을 때까지 대기하는 방식으로 해결해 보았습니다.

kafka 의 경우 producer 가 subscribe 완료 여부를 알수 없어서 양방향으로 메시지를 주고 받도록 하고, 응답 메시지를 받기까지 Future 객체를 통해 스레드가 대기하도록 하였습니다. 대략적인 구조는 아래와 같습니다.   

| ![future](./img/future.png) |
|:---:|
| 양방향 Kafka 통신과 Future 를 이용한 대기 처리 |

- 응답을 받아야 하기 때문에 kafka 를 양방향으로 사용합니다.
  + `command` -> `query`: Event 전달
  + `query` -> `command`: 반영이 완료되었다는 응답 (Ack message)
- 대기를 위해 Future 객체를 사용하여 Thread.sleep() 을 수행합니다.
- 로그인 시도하는 유저에 대한 ID 를 key 로 하는 전역 map 에 Future 를 저장합니다. 
- Aspect 와 TransactionSynchronizationManager 를 통해 도메인 로직과 대기 로직을 분리합니다.
